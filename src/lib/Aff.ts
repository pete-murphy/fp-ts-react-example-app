...
function chain_<A, B>(ma: Aff<A>, amb: (a: A) => Aff<B>): Aff<B> {
  return (e, s) => {
    let c1: Canceler
    let c2: Canceler
    let isCanceled = false
    let requestCancel = false
    let onCanceler: (canceler: Canceler) => void = () => void 0
...
export const aff: Monad1<URI> & MonadTask1<URI> = {
  ...
  of,
  ap: (mab, ma) => chain_(mab, (f) => (e, s) => ma(e, (a) => s(f(a)))), // Derived
  chain: chain_,
  fromIO,
...
const { ap, apFirst, apSecond, chain, chainFirst, flatten, map } = pipeable(aff)
